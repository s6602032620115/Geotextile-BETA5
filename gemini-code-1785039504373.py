...
def generate_word_report():
    ...
    # Title
    title = doc.add_heading('รายงานการคำนวณออกแบบกำแพงกันดินเสริมกำลัง Geotextile', level=0)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    
    doc.add_paragraph('ระบบออกแบบอัตโนมัติ Geotextile Reinforced Wall Designer Pro')
    doc.add_paragraph('----------------------------------------------------------------------------------------------------')
    
    # Section 1: Inputs
    doc.add_heading('1. ข้อมูลนำเข้าสำหรับการออกแบบ (Design Parameters)', level=1)
    p1 = doc.add_paragraph()
    p1.add_run(f'• ความสูงกำแพง (H): {H:.2f} m\n')
    p1.add_run(f'• ระยะเรียง Geotextile แนวดิ่ง (Sv): {Sv:.2f} m\n')
    p1.add_run(f'• คุณสมบัติดินถม: Unit weight = {gamma1:.2f} kN/m³, Phi = {phi1:.1f}°\n')
    p1.add_run(f'• คุณสมบัติดินฐานราก: Unit weight = {gamma2:.2f} kN/m³, Phi = {phi2:.1f}°\n')
    p1.add_run(f'• Geotextile Ultimate Strength (T_ult): {T_ult:.2f} kN/m\n')
    p1.add_run(f'• รวมค่า Reduction Factors (RF_total): {RF_total:.2f}')
    
    # Section 2: Results
    doc.add_heading('2. ผลการคำนวณและตรวจสอบเสถียรภาพ (Calculations & Safety Checks)', level=1)
    
    table = doc.add_table(rows=1, cols=4)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    hdr_cells = table.rows[0].cells
    hdr_cells[0].text = 'รายการตรวจสอบ'
    hdr_cells[1].text = 'ค่าที่คำนวณได้ (FS)'
    hdr_cells[2].text = 'เกณฑ์ขั้นต่ำ'
    hdr_cells[3].text = 'ผลการประเมิน'
    
    data = [
        ('1. การพลิกคว่ำ (Overturning)', f'{FS_overturning:.2f}', '≥ 2.0', 'PASS' if is_ot_pass else 'FAIL'),
        ('2. การลื่นไถล (Sliding)', f'{FS_sliding:.2f}', '≥ 1.5', 'PASS' if is_sl_pass else 'FAIL'),
        ('3. กำลังรับน้ำหนักฐานราก (Bearing)', f'{FS_bearing:.2f}', '≥ 3.0', 'PASS' if is_be_pass else 'FAIL')
    ]
    
    for item, fs_val, req, status in data:
        row_cells = table.add_row().cells
        row_cells[0].text = item
        row_cells[1].text = fs_val
        row_cells[2].text = req
        row_cells[3].text = status

    doc.add_paragraph('\n')
    doc.add_heading('3. สรุปผลทางวิศวกรรม (Engineering Summary)', level=1)
    
    summary_text = f"จากการคำนวณ ความยาวแผ่นสังเคราะห์ Geotextile ที่ต้องใช้ (L) เท่ากับ {L:.2f} เมตร "
    if all_pass:
        summary_text += "โครงสร้างกำแพงกันดินเสริมกำลังนี้ ผ่านเกณฑ์ความปลอดภัยตามมาตรฐานวิศวกรรมปฐพีทุกรายการ สามารถนำไปใช้ในการก่อสร้างจริงได้"
    else:
        summary_text += "พบรายการที่ไม่ผ่านเกณฑ์ความปลอดภัย โปรดปรับเปลี่ยนขนาดความยาว L หรือเลือกใช้เกรด Geotextile ที่มีรับแรงดึงได้สูงขึ้น"
...
with col_left:
    ...
    st.download_button(
        label="📝 ดาวน์โหลดรายงานผลการคำนวณ (.docx)",
        data=doc_file,
        file_name=f"Geotextile_Wall_Design_Report_H{H}m.docx",
    ...
    aoi_msg = "ยอดเยี่ยมมากค่ะ! ค่าคำนวณผ่านเกณฑ์ความปลอดภัยทั้งหมด โครงสร้างแข็งแรงสมบูรณ์แล้วค่ะ ✨ สามารถกดปุ่มโหลดรายงาน Word ด้านบนได้เลยนะคะ!" if all_pass else "ว้า... มีบางเกณฑ์ยังไม่ผ่านความปลอดภัยนะคะ! ลองเพิ่มความยาว L หรือเปลี่ยนเกรด Geotextile เพิ่มเติมดูนะคะ 💡"
...
with col_right:
    ...
    with tab1:
        ...
        for i in range(0, num_layers):
            ...
            # Concrete Blocks Facing
            layers_svg += f'<rect x="{ox - 16}" y="{ly_y - layer_h_px}" width="16" height="{layer_h_px}" fill="url(#blockGrad)" stroke="#475569" stroke-width="0.8" rx="1.5" filter="url(#shadow)"/>'
            
            # Geotextile Line
            if i > 0:
                layers_svg += f'<line x1="{ox}" y1="{ly_y}" x2="{ox + w_px}" y2="{ly_y}" stroke="url(#geoGrad)" stroke-width="3.5" stroke-dasharray="10,4" class="animated-geo" />'
                # Tension Particle Dots
                layers_svg += f'<circle cx="{ox + w_px/2}" cy="{ly_y}" r="2.5" fill="#f59e0b" class="pulse-dot"/>'
        ...
        <svg width="100%" height="{svg_h}" viewBox="0 0 {svg_w} {svg_h}" xmlns="http://www.w3.org/2000/svg" style="background:#090d16; border-radius:16px; border:1px solid #1e293b; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
            <defs>
                ...
                <pattern id="soilPattern" width="24" height="24" patternUnits="userSpaceOnUse">
                    ...
                    <circle cx="4" cy="4" r="1.5" fill="#38bdf8" opacity="0.25"/>
                    <circle cx="16" cy="16" r="2" fill="#38bdf8" opacity="0.15"/>
                ...
                <pattern id="foundPattern" width="16" height="16" patternUnits="userSpaceOnUse">
                    ...
                    <path d="M 0 16 L 16 0" stroke="#334155" stroke-width="1.5"/>
                </pattern>
            ...
            <rect x="{ox - 60}" y="{oy}" width="{w_px + 130}" height="75" fill="url(#foundPattern)" rx="4"/>
            <line x1="{ox - 60}" y1="{oy}" x2="{ox + w_px + 70}" y2="{oy}" stroke="#475569" stroke-width="2"/>
            <text x="{ox + (w_px/2) - 50}" y="{oy + 45}" fill="#94a3b8" font-family="sans-serif" font-size="12" font-weight="bold">Foundation Soil (γ2, φ2)</text>
            ...
            <text x="{ox + 18}" y="{oy - (h_px/2)}" fill="#38bdf8" font-family="sans-serif" font-size="13" font-weight="bold" opacity="0.9">Reinforced Soil (γ1, φ1)</text>

            <!-- Geotextile Layers & Facing Blocks -->
            {layers_svg}

            <!-- Cap Block -->
            <rect x="{ox - 18}" y="{oy - h_px - 8}" width="20" height="8" fill="#f87171" stroke="#ffffff" stroke-width="0.8" rx="1" filter="url(#shadow)"/>

            <!-- Active Earth Pressure Wedge & Animated Force -->
            <g class="animated-pressure">
                <polygon points="{ox + w_px},{oy} {ox + w_px},{oy - h_px} {ox + w_px + 50},{oy}" fill="url(#pressGrad)" stroke="#ef4444" stroke-width="1.5" stroke-dasharray="4,2"/>
                
                <!-- Force Arrow & Label -->
                <line x1="{ox + w_px + 40}" y1="{oy - (h_px/3)}" x2="{ox + w_px + 5}" y2="{oy - (h_px/3)}" stroke="#ef4444" stroke-width="3" marker-end="url(#arrow)" filter="url(#glow)"/>
                <text x="{ox + w_px + 8}" y="{oy - (h_px/3) - 12}" fill="#ef4444" font-family="sans-serif" font-size="12" font-weight="bold">Pa (Active Earth Pressure)</text>

                <!-- Pressure Streamlines -->
                <line x1="{ox + w_px + 30}" y1="{oy - (h_px*0.6)}" x2="{ox + w_px + 5}" y2="{oy - (h_px*0.6)}" stroke="#ef4444" stroke-width="1.5" stroke-dasharray="4,4" class="pressure-particles" opacity="0.7"/>
                <line x1="{ox + w_px + 20}" y1="{oy - (h_px*0.8)}" x2="{ox + w_px + 5}" y2="{oy - (h_px*0.8)}" stroke="#ef4444" stroke-width="1.5" stroke-dasharray="4,4" class="pressure-particles" opacity="0.5"/>
            </g>
            ...
            <!-- Dimension L -->
            <line x1="{ox}" y1="{oy + 25}" x2="{ox + w_px}" y2="{oy + 25}" stroke="#38bdf8" stroke-width="1.5" marker-start="url(#dimArrow)" marker-end="url(#dimArrow)"/>
            <text x="{ox + (w_px/2) - 30}" y="{oy + 44}" fill="#38bdf8" font-family="sans-serif" font-size="13" font-weight="bold">L = {L:.2f} m</text>
        </svg>
        """
        components.html(svg_code, height=500)

    with tab2:
        # Mascot Helper (Ken-kun)
        st.markdown("""
